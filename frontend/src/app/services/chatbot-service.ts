...
import { ChatReqDTO, ChatMessage, ScenarioReqDTO, TradCorrigeeReqDto, TradRapidesResult } from '../models/chatbot.model';
...
export class ChatbotService {
  ...
  genChat(req: ChatReqDTO, endpoint: ApiEndpoint): Observable<string> {
    return this.http
      .post(`${this.baseUrl}/${endpoint}`, req, { responseType: 'text' })
      .pipe(
        tap((res) => {
          res = res.replace(/\*/g, '');
  ...
  genScenario(req: ScenarioReqDTO, endpoint: ApiEndpoint, introOnly = false): Observable<string> {
    return this.http
      .post(`${this.baseUrl}/${endpoint}`, req, { responseType: 'text' })
      .pipe(
        tap((res) => {
          res = res.replace(/\*/g, '');
          if (introOnly) {
            this.resList.update((prev) => [...prev, { sender: 'Ai', message: res }]);
          } else {
            this.addResponse(req.message, res);
          }
          this.ttsService.speak(res);
